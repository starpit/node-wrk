...
/** latency stacks */
const parseLatencyStacks = (result, lines) => {
    const idx = lines.findIndex(line => line.indexOf('latencyStacks') >= 0)
    if (idx > 0) {
        try {
            const { latencyStacks } = JSON.parse(lines[idx])
            result.latencyStacks = latencyStacks
        } catch (err) {
            console.error(err)
        }
        lines.splice(idx, 1)
    } else {
        console.error("hmmm...", lines)
    }
...
function parseWrk(stdout) {
  ...
  var result = {};
  lines = parseLatencyStacks(result, lines)
  ...
  result.latencyMin   = latencyMinMax[6];
  ...
  result.rpsMin   = rpsMinMax[6];

  if (lines[5].match(/Latency Distribution/)) {
    result.latency25 = lines[6].split(/[\t ]+/)[2];
    result.latency50 = lines[7].split(/[\t ]+/)[2];
    result.latency75 = lines[8].split(/[\t ]+/)[2];
    result.latency90 = lines[9].split(/[\t ]+/)[2];
    result.latency99 = lines[10].split(/[\t ]+/)[2];
  }
